...
export default function TravelMap({ visitedPlaces, onPlaceToggle, onCitiesUpdate }: TravelMapProps) {
  ...
  // Função para diagnosticar e corrigir dados de países
  const diagnoseAndFixCountries = () => {
    console.log('🔍 DEBUG: Iniciando diagnóstico de países...')
    
    // Verificar dados atuais
    console.log('📊 Cidades atuais:', visitedCities)
    
    // Verificar países únicos
    const uniqueCountries = new Set(visitedCities.map(city => city.country))
    console.log('🌍 Países únicos encontrados:', Array.from(uniqueCountries))
    console.log('📈 Total de países:', uniqueCountries.size)
    
    // Verificar se há países undefined, null ou vazios
    const invalidCountries = visitedCities.filter(city => 
      !city.country || 
    ...
    if (invalidCountries.length > 0) {
      console.log('⚠️ Cidades com países inválidos:', invalidCountries)
    }
    
    // Verificar duplicatas ou variações de nomes
    const countryVariations = visitedCities.reduce((acc, city) => {
      if (city.country) {
        const normalized = city.country.trim().toLowerCase()
        if (!acc[normalized]) {
          acc[normalized] = []
        }
        acc[normalized].push(city)
      }
      return acc
    }, {} as Record<string, VisitedCity[]>)
    
    console.log('🔄 Variações de países encontradas:', countryVariations)
    
    // Verificar se há países que deveriam ser o mesmo
    const suspiciousCountries = Object.entries(countryVariations).filter(([, cities]) => {
      return cities.length === 1 && cities[0].country !== cities[0].country.trim()
    })
    
    if (suspiciousCountries.length > 0) {
      console.log('🤔 Países suspeitos (possíveis duplicatas):', suspiciousCountries)
    }
    ...
    return {
      ...
      invalidCountries: invalidCountries.length,
      suspiciousCountries: suspiciousCountries.length
    }
  }

  // Função para limpar e normalizar dados de países
  const cleanAndNormalizeCountries = () => {
    console.log('🧹 DEBUG: Iniciando limpeza e normalização de países...')
    
    const cleanedCities = visitedCities.map(city => {
      const cleanedCity = { ...city }
      
      // Normalizar país
      if (city.country) {
        let normalizedCountry = city.country.trim()
        
        // Corrigir variações comuns
        if (normalizedCountry.toLowerCase() === 'brasil' || 
            normalizedCountry.toLowerCase() === 'brazil') {
          normalizedCountry = 'Brasil'
        } else if (normalizedCountry.toLowerCase() === 'argentina') {
          normalizedCountry = 'Argentina'
        } else if (normalizedCountry.toLowerCase() === 'paraguai' || 
                   normalizedCountry.toLowerCase() === 'paraguay') {
          normalizedCountry = 'Paraguai'
        } else if (normalizedCountry.toLowerCase() === 'bolivia' || 
                   normalizedCountry.toLowerCase() === 'bolívia') {
          normalizedCountry = 'Bolívia'
        } else if (normalizedCountry.toLowerCase() === 'chile') {
          normalizedCountry = 'Chile'
        }
        
        cleanedCity.country = normalizedCountry
      }
      
      return cleanedCity
    })
    
    // Remover duplicatas baseadas em nome e país
    const uniqueCities = cleanedCities.filter((city, index, self) => 
      index === self.findIndex(c => 
        c.name === city.name && c.country === city.country
      )
    )
    
    console.log('🧹 Cidades antes da limpeza:', visitedCities.length)
    console.log('🧹 Cidades após limpeza:', uniqueCities.length)
    
    // Atualizar estado
    setVisitedCities(uniqueCities)
    saveCitiesToStorage(uniqueCities)
    
    // Notificar página principal
    if (onCitiesUpdate) {
      onCitiesUpdate(uniqueCities)
    }
  ...
  return (
    <div className="w-full h-96 bg-gray-100 rounded-lg overflow-hidden relative">
      ...
      <div className="absolute top-4 right-4 z-10">
        ...
        {/* Botão para diagnosticar países */}
        <button
          onClick={() => {
            console.log('🔍 Executando diagnóstico de países...')
            const diagnosis = diagnoseAndFixCountries()
            console.log('📊 Resultado do diagnóstico:', diagnosis)
          }}
          className="mt-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-2 transition-colors w-full"
        >
          <span className="text-lg">🔍</span>
          Diagnosticar Países
        </button>

        {/* Botão para limpar e normalizar países */}
        <button
          onClick={() => {
            console.log('🧹 Executando limpeza e normalização...')
            const cleanedCities = cleanAndNormalizeCountries()
            console.log('✅ Limpeza concluída:', cleanedCities.length, 'cidades')
          }}
          className="mt-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-2 transition-colors w-full"
        >
          <span className="text-lg">🧹</span>
          Limpar e Normalizar Países
        </button>
